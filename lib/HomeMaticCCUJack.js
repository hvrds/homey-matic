...
class HomeMaticCCUJack extends EventEmitter {
    ...
    async listDevices() {
        ...
        self.logger.log('info', '🔍 DEBUG:', 'Starting device list from CCU-Jack');

        return new Promise((resolve, reject) => {
            self.jackClient.get('device')
                .then(resp => {
                    let deviceLinks = resp.data['~links']
                    self.logger.log('info', '🔍 DEBUG:', 'Device links count:', deviceLinks.length);
                    ...
                    Promise.all(devPromises)
                        .then(results => {
                            ...
                            for (var i = 0; i < results.length; i++) {
                                ...
                                self.logger.log('info', '🔍 DEBUG:', 'Device type:', dev.type, 'Interface:', dev.interfaceType, 'Has address:', !!dev.address);
                                
                                if (dev.type === 'HmIP-MOD-TM') {
                                    self.logger.log('info', '🔍 DEBUG:', '📦 HmIP-MOD-TM device data:', JSON.stringify(dev));
                                }
                            ...
                            self.logger.log('info', '🔍 DEBUG:', '📦 Final structure by interface:', Object.keys(devices).map(iface => {
                                return iface + ': ' + devices[iface].length + ' devices';
                            }).join(', '));
                            
                            resolve(devices)
                        })
                        .catch(err => {
                            self.logger.log("error", "🔍 DEBUG:", "⚠️ Failed to process devices:", err)
                            self.logger.log("error", "Failed to process devices:", err)
                            reject(err)
                        })
                })
                .catch(err => {
                    self.logger.log("error", "🔍 DEBUG:", "⚠️ Failed to get devices:", err)
